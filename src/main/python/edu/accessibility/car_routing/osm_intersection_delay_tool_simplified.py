...
def process_speed_limits(link_gdf, road_type_col='highway', known_speed_limit_col='maxspeed',
    ...
    data.crs = link_gdf.crs  # after the apply, geodataframe is lossing its crs
...
def getSpeedHigherModeValue(speeds):
    ...
    if isinstance(speeds, list):
        if len(speeds) == len(set(speeds)):
            ...
            medianPosInt = int(len(speeds) / 2)
            if len(speeds) > 2 and (medianPos - medianPosInt) == 0:
    ...
    for index, row in data.iterrows():
        for column in data.columns:
            if isinstance(row[column], list):
                ...
                columnsAreList.append(column)

    # print("ShpName:", len(join.loc[join.apply(lambda row: True if isinstance(row.ShpName, list) else False, axis=1)]))
    # print("freeflow:", len(join.loc[join.apply(lambda row: True if isinstance(row.freeflow, list) else False, axis=1)]))
    # print("geometry:", len(join.loc[join.apply(lambda row: True if isinstance(row.geometry, list) else False, axis=1)]))
    # print("highway:", len(join.loc[join.apply(lambda row: True if isinstance(row.highway, list) else False, axis=1)]))
    # print("jtype1:", len(join.loc[join.apply(lambda row: True if isinstance(row.jtype1, list) else False, axis=1)]))
    # print("jtype2:", len(join.loc[join.apply(lambda row: True if isinstance(row.jtype2, list) else False, axis=1)]))
    # print("jtype3:", len(join.loc[join.apply(lambda row: True if isinstance(row.jtype3, list) else False, axis=1)]))
    # print("jtype4:", len(join.loc[join.apply(lambda row: True if isinstance(row.jtype4, list) else False, axis=1)]))
    # print("jtype5:", len(join.loc[join.apply(lambda row: True if isinstance(row.jtype5, list) else False, axis=1)]))
    # print("junction:", len(join.loc[join.apply(lambda row: True if isinstance(row.junction, list) else False, axis=1)]))
    # print("length:", len(join.loc[join.apply(lambda row: True if isinstance(row.length, list) else False, axis=1)]))
    # print("maxspeed:", len(join.loc[join.apply(lambda row: True if isinstance(row.maxspeed, list) else False, axis=1)]))
    # print("new_length:", len(join.loc[join.apply(lambda row: True if isinstance(row.new_length, list) else False, axis=1)]))
    # print("old_osmid:", len(join.loc[join.apply(lambda row: True if isinstance(row.old_osmid, list) else False, axis=1)]))
    # print("oneway:", len(join.loc[join.apply(lambda row: True if isinstance(row.oneway, list) else False, axis=1)]))
    # print("osmid:", len(join.loc[join.apply(lambda row: True if isinstance(row.osmid, list) else False, axis=1)]))
    # print("x:", len(join.loc[join.apply(lambda row: True if isinstance(row.x, list) else False, axis=1)]))
    # print("y:", len(join.loc[join.apply(lambda row: True if isinstance(row.y, list) else False, axis=1)]))
...
def main():
    ...
    data = process_speed_limits(link_gdf=edges,
                                # limit_gdf=limits,
    ...
    #### checking columns
    # """
    # Index(['KmH', 'ShpName', 'freeflow', 'geometry', 'highway', 'jtype1', 'jtype2',
    #    'jtype3', 'jtype4', 'junction', 'length', 'maxspeed', 'new_length',
    #    'old_osmid', 'oneway', 'osmid', 'x', 'y', 'jtype5'],
    #   dtype='object')
    # """
    # indexes, columns = checkRowValueTypes(join)
    # print("Indexes: ", indexes)
    # print("Columns: ", columns)
    # # print("Indexes:", join.loc[join.apply(lambda row: True if checkRowValueTypes(row) else False, axis=1)].index.values)
